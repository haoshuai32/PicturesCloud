...
app = Sanic(name="haoshuai")
# 图片存储目录
baseDir = "/Users/haoshuai/Documents/photoprism/"#sys.path[0] + '/image/'
...
def getSuffix(filename):
    ...
    suffix = tempArr[-1]
    fileType = ['jpg', 'jpeg', 'gif', 'png','HEIC']
    if len(tempArr) < 2:
        return 'error name'
...
def checkHost(host):
    for i in allowHost:
        if i in host:
            return True
    return False
# File(type='jpeg', body=b'hello my body text', name='pimage.jpeg')
# 上传图片文件接口
...
async def upimg(request):
    ...
    # print(request.files)
    item = request.files.get("original")
    # print("myitme xxxxxxxxxxxxxx", item,type(item))
    # print(item.body)
    # print("my FIle",item.get("file"))
